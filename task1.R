...
BiocManager::install(c("DESeq2", "Biobase", "GEOquery", "limma"))

# CRAN packages
install.packages(c("data.table", "dplyr", "gplots", "umap", "ggplot2", "RColorBrewer"))
...
path <- paste(urld, "acc=GSE158674", "file=GSE158674_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
gs <- factor(sml)
groups <- make.names(c("GBM39","HK301","HK296"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE158674 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE158674 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE158674", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
